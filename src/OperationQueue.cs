...
namespace Afonsoft.Parallel
{
    ...
    public class OperationQueue<TWorker, TContext> where TWorker : Worker<TContext>, new()
    {
        ...
        private void Initialize()
        {
            ...
            this.pool = new WorkerPool(new EventHandler<TaskEventArgs>(this.WorkerPool_Pump), MaxNumberOfWorkers);
            ...
            {
                this.pool.AddWorker(index.ToString(), (WorkerControllerBase)new WorkerController<TWorker>());
        ...
        /// Stop
        /// </summary>
        /// <param name="waitQueueCompletion">Esperar finalizar o processo atual</param>
        public void Stop(bool waitQueueCompletion)
        {
            ...
            this.isQueueStopping = false;
            this.isQueueStarted = false;
            this.WaitForOperationQueueCompletion();
            this.pool.Stop();
        }
                ...
                switch (e.NotificationOf)
                {
                    ...
                    case TaskNotification.WorkerError:
                        ...
                        lock (this.queueSyncObject)
                        {
                            ...
                            this.pool.AddWorker(e.CorrelationState, (WorkerControllerBase)new WorkerController<TWorker>());
                            this.roundRobinQueue.Enqueue(e.CorrelationState);
                        }
                        this.PumpOperation();
                        break;
